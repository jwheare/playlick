...
function CouchDB(name, httpHeaders) {
  this.name = name;
  this.uri = "/data/" + encodeURIComponent(name) + "/";
...
CouchDB.allDbs = function() {
  CouchDB.last_req = CouchDB.request("GET", "/data/_all_dbs");
...
CouchDB.getVersion = function() {
  CouchDB.last_req = CouchDB.request("GET", "/data/");
  CouchDB.maybeThrowError(CouchDB.last_req);
...
CouchDB.replicate = function(source, target, rep_options) {
  ...
  CouchDB.last_req = CouchDB.request("POST", "/data/_replicate", {
    headers: headers,
...
CouchDB.requestStats = function(module, key, test) {
  ...
  var stat = CouchDB.request("GET", "/data/_stats/" + module + "/" + key + query_arg).responseText;
...
CouchDB.newUuids = function(n) {
  if (CouchDB.uuids_cache.length >= n) {
    ...
    CouchDB.last_req = CouchDB.request("GET", "/data/_uuids?count=" + (100 + n));
    CouchDB.maybeThrowError(CouchDB.last_req);
